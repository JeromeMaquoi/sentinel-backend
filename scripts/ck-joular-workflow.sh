...
if [ -z "$CLONE" ] || [ "$CLONE" == "true" ]; then
    log_and_print_output_with_date "Start cloning all repositories"
...
if [ -z "$CK" ] || [ "$CK" == "true" ]; then
    log_and_print_output_with_date "Start CK analysis for all directories"
...
if [ -z "$JOULAR" ] || [ "$JOULAR" == "true" ]; then
    log_and_print_output_with_date "Start JoularJX analysis for all directories"
