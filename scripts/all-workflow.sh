...
echo -e "-------------------------------------------------"
echo -e "-------------------------------------------------"
REPO_DIRECTORY="$REPO_DIRECTORY" PLUGINS_DIRECTORY="$PLUGINS_DIRECTORY" BATCH_SIZE="$BATCH_SIZE" GITHUB_TOKEN="$GITHUB_TOKEN" NB_ITERATION="$NB_ITERATION" REMOVE_REPO="$REMOVE_REPO" CLONE="$CLONE" CK="$CK" JOULAR="$JOULAR" bash ./ck-joular-workflow.sh
